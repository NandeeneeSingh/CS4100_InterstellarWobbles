...
OUTPUT_CSV = Path("data/03_physics_anchored/synthetic_ns_mc_expanded.csv")
...
def main():
    ...
    # Synthesize neutron stars
    df = df.copy()
    df["source_id"] = df["source_id"].astype(str)
    df.loc[:, "m2_true"] = rng.uniform(NS_MASS_MIN, NS_MASS_MAX, size=n_systems)
    df.loc[:, "source_id"] = "SYNTH_NS_" + df["source_id"]
    ...
    P_days = clone_df["period"].values
    e = clone_df["eccentricity"].clip(lower=0, upper=0.999).values # make sure e != 1

    denominator = CONVERSION_CONSTANT * P_days * (1.0 - e**2)**1.5
